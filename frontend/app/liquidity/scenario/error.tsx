...
export default function ScenarioPlannerError({
  error,
  reset,
}: {
      ...
      <Card className="w-full max-w-md border-rose-200 dark:border-rose-900">
        <CardHeader className="text-center pb-3">
          ...
          <CardTitle className="text-lg">
            {isAr ? "حدث خطأ في مخطط السيناريوهات" : "Scenario Planner Error"}
          </CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <p className="text-sm text-muted-foreground text-center">
            {isAr
              ? "عذراً، حدث خطأ أثناء تحميل مخطط السيناريوهات. يرجى المحاولة مرة أخرى."
              : "Sorry, an error occurred while loading the scenario planner. Please try again."}
          </p>
