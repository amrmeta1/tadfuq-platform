...
type BankProvider interface {
	...
	FetchAccounts(ctx context.Context, tenantID uuid.UUID, credentials map[string]string) ([]domain.BankAccount, error)

	// FetchTransactions retrieves transactions for the given account from the provider.
	FetchTransactions(ctx context.Context, tenantID uuid.UUID, accountID uuid.UUID, credentials map[string]string) ([]domain.BankTransaction, error)
}
...
func (s *StubBankProvider) FetchAccounts(_ context.Context, _ uuid.UUID, _ map[string]string) ([]domain.BankAccount, error) {
	// TODO: Implement real bank account fetching via Lean/Tarabut API
	return nil, nil
}

func (s *StubBankProvider) FetchTransactions(_ context.Context, _ uuid.UUID, _ uuid.UUID, _ map[string]string) ([]domain.BankTransaction, error) {
