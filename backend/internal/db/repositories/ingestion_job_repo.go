...
func (r *IngestionJobRepo) Create(ctx context.Context, tenantID uuid.UUID, input models.CreateIngestionJobInput) (*models.IngestionJob, error) {
	...
	err := r.pool.QueryRow(ctx, `
		...
		tenantID, input.JobType, func() []byte { b, _ := json.Marshal(input.Metadata); return b }(),
	).Scan(
...
func (r *IngestionJobRepo) GetByID(ctx context.Context, id uuid.UUID) (*models.IngestionJob, error) {
	var job models.IngestionJob
	err := r.pool.QueryRow(ctx, `
...
func (r *IngestionJobRepo) UpdateStatus(ctx context.Context, id uuid.UUID, status models.JobStatus, errMsg string) error {
	now := time.Now().UTC()
	var query string
	var args []any
